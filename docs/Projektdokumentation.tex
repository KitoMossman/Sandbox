...
\item{Hardware-Effizienz:} Das primäre Web-Hosting-Stack (CloudPanel, Nginx, PHP, MariaDB) muss nativ auf dem Bare-Metal-Host (Ubuntu 24.04 LTS) betrieben werden, um Virtualisierungs-Overhead zu vermeiden und die Leistung auf der limitierten Hardware zu maximieren.
...
\item{Zentrale Telemetrie und Recovery:} Es muss ein zentrales Web-Dashboard (Infrastructure Hub) geschaffen werden, das Administratoren (und Lehrkräften) in Echtzeit die Systemlast anzeigt, Fehlerzustände visualisiert und eine One-Click-Wiederherstellung von Containern (Soft Reset) oder der gesamten Host-Partition (Hard Reset via partclone) ermöglicht.
...
\paragraph{Grundsystem}

\paragraph{Docker - Engine}

\paragraph{Cloudpanel}
